...
**Run type:** Normal weekly run. Prior report: [`weekly-cache-report-2026-04-21.md`](./weekly-cache-report-2026-04-21.md). Search horizon: 2026-04-20 → 2026-04-26 (past 7 days, ICLR 2026 week). Target was ~5 entries; expanded to 7 to avoid omitting strong items in a busy week.
...
### 1. DeepSeek-V4-Pro tech report: CSA + HCA hybrid attention for 1M-token KV (Apr 24, 2026)

- Reference: DeepSeek-AI, [DeepSeek_V4.pdf on Hugging Face](https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro/blob/main/DeepSeek_V4.pdf) ([model card](https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro), [API release note](https://api-docs.deepseek.com/news/news260424)).
- Summary: Preview release of a 1.6T-parameter MoE model designed from the ground up around 1M-token contexts. The KV-cache stack is a hybrid of two new mechanisms: **Compressed Sparse Attention (CSA)** — 4× KV compression followed by query-dependent top-1024 selection over the compressed entries — and **Heavily Compressed Attention (HCA)** — 128× KV compression with full dense attention over the (now very short) compressed sequence. Layers are assigned to one of the two mechanisms.
- Novelty: High. CSA and HCA are not minor tweaks of MLA / native sparse attention; they reframe long-context as "design the compressor and the attention pattern jointly per layer." HCA in particular is unusual — most KV compressors keep attention sparse, HCA goes the other way.
- Impact: Very high if reproducible. A frontier-scale, openly released model that publishes both the mechanism *and* the inference cost numbers is the rare combination that lets the rest of the field validate against something real. Already provoking immediate "needle-in-a-haystack" scrutiny re: the 128× compression tier.
- Runtime evaluation: Yes — KV cache reduced to **~10%** and inference FLOPs to **~27%** of DeepSeek-V3.2 at 1M-token scale, with quality benchmarks vs. V3.2 in the report. Latency / throughput on production-class fleets are summarised rather than fully broken down (TTFT/TBT curves, batched-decoding numbers are partial). Independent reproductions of the inference cost claim are still owed.

### 2. KServe × llm-d × vLLM: prefix-cache-aware routing measured on Llama 3.1 70B / MI300X (Apr 21, 2026)
...
### 3. SAW-INT4: System-Aware 4-Bit KV-Cache Quantization for Real-World LLM Serving (Apr 21, 2026)
...
### 4. TurboQuant: Online Vector Quantization with Near-Optimal Distortion Rate (ICLR 2026, presented Apr 25, 2026)
...
### 5. SparKV: Overhead-Aware KV Cache Loading for Efficient On-Device LLM Inference (Apr 23, 2026)
...
### 6. How Much Cache Does Reasoning Need? Depth–Cache Tradeoffs in KV-Compressed Transformers (Apr 20, 2026)
...
- Impact: Medium-high near term, foundational long-term. Direct prescriptive value for KV-compression designers: aggressive compression imposes a depth tax on multi-hop reasoning that no clever controller can erase — this directly bears on the safety margin of DeepSeek-V4-style 128× HCA tiers.
- Runtime evaluation: None (theory paper). Worth reading alongside the empirical "Hold Onto That Thought" reasoning-quality benchmarks ([arXiv:2512.12008](https://arxiv.org/abs/2512.12008)).

---

## A′. Closely related work just outside the 7-day window

### 7. Prefill-as-a-Service (PrfaaS): cross-datacenter KVCache for next-gen models (Apr 16, 2026)

- Reference: Qin et al. (Moonshot AI / Tsinghua), [arXiv:2604.15039](https://arxiv.org/abs/2604.15039). Discussion: [Hacker News thread](https://news.ycombinator.com/item?id=47822117).
- Summary: Pushes PD disaggregation across datacenter boundaries: long-context prefill is selectively offloaded to compute-dense remote prefill clusters, and the resulting KVCache is shipped over commodity Ethernet (not RDMA) to the local PD cluster for decode. Combines model-side KV efficiency (smaller per-token KV) with system-side selective offloading, bandwidth-aware scheduling, and cache-aware request placement.
- Novelty: High as systems direction. Drops the implicit "prefill and decode share an RDMA fabric" assumption that has shaped Mooncake / Dynamo / llm-d, enabling independent scaling of heterogeneous prefill and decode capacity across loosely coupled clusters.
- Impact: Very high for hyperscale serving. Directly enables the kind of 1M-context economics DeepSeek-V4 (entry #1) is selling — moving long-context prefill to wherever the cheapest accelerators are, regardless of locality.
- Runtime evaluation: Yes — case study on an internal 1T-parameter hybrid model: **+54% serving throughput vs. homogeneous PD** and **+32% vs. naive heterogeneous baseline**, with modest cross-DC bandwidth use. Production-trace level rather than full SLO study.

---

## Cross-cutting observations (this week)

- **Frontier-model releases are now publishing KV-cache mechanisms as a first-class artifact.** DeepSeek-V4-Pro lands the same week as TurboQuant and SAW-INT4 with two new attention/compression mechanisms (CSA, HCA) framed primarily by their KV-cache and FLOPs reductions at 1M tokens. The model release *is* the cache paper.
...
- **Disaggregation is going cross-datacenter.** PrfaaS removes the RDMA-fabric assumption that has shaped Mooncake / Dynamo / llm-d; combined with DeepSeek-V4's smaller-per-token KV, it makes long-context prefill economically separable from decode. Expect more "pool prefill across regions" papers to follow.
...
- **Runtime-evaluation gap persists at both ends of the pipeline.** TurboQuant publishes attention-fidelity + microbench throughput but no continuous-batching TTFT/TBT/SLO; DeepSeek-V4 publishes top-line KV-size and FLOPs reductions but partial latency curves. Independent reproductions are the obvious next step on both.

## References

- [DeepSeek-V4 tech report (PDF)](https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro/blob/main/DeepSeek_V4.pdf) · [DeepSeek-V4-Pro model card](https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro) · [DeepSeek API release note (Apr 24, 2026)](https://api-docs.deepseek.com/news/news260424)
- [llm-d — Production-Grade LLM Inference at Scale with KServe, llm-d, and vLLM (Apr 21, 2026)](https://llm-d.ai/blog/production-grade-llm-inference-at-scale-kserve-llm-d-vllm) · [Red Hat Developer — Combining KServe and llm-d (Apr 21, 2026)](https://developers.redhat.com/articles/2026/04/21/kserve-llm-d-optimized-gen-ai-inference)
...
- [arXiv:2604.17935 — Depth–Cache Tradeoffs (Apr 20, 2026)](https://arxiv.org/abs/2604.17935) · [arXiv:2512.12008 — Hold Onto That Thought (empirical companion)](https://arxiv.org/abs/2512.12008)
- [arXiv:2604.15039 — Prefill-as-a-Service / PrfaaS (Apr 16, 2026)](https://arxiv.org/abs/2604.15039)
...
- [arXiv:2604.04722 — Don't Waste Bits! (adaptive on-device KV-quant)](https://arxiv.org/abs/2604.04722) — supporting evidence for the edge-KV trend noted above.
